#!/bin/zsh
set -e

cd "/Users/tomasz/Documents/Codex/2026-05-15/can-you-build-a-website-for"

OUT="content-elevated-product-os/exports/customer-pdf-hair-stylist-flagship-v1"
PY="/Users/tomasz/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3"

echo "Exporting Hair Stylist flagship bundle into: $OUT"
echo ""

python3 scripts/export_customer_pdfs_outside_codex.py \
  --batch phase-1 \
  --only hair-stylists \
  --html-pattern "*.html" \
  --output "$OUT" \
  --slow \
  --timeout 120

python3 scripts/validate_customer_pdf_export.py \
  --batch phase-1 \
  --only hair-stylists \
  --output "$OUT"

python3 scripts/audit_pdf_layout_risk.py \
  --batch phase-1 \
  --only hair-stylists \
  --output "$OUT"

"$PY" scripts/create_pdf_contact_sheets.py \
  --root "$OUT" \
  --batch phase-1 \
  --only hair-stylists \
  --max-pages 45

python3 scripts/build_pdf_visual_review_index.py \
  --root "$OUT" \
  --batch phase-1 \
  --title "Hair Stylist Flagship Visual Review"

echo ""
echo "Hair Stylist export complete."
echo "Review dashboard:"
echo "$OUT/phase-1-visual-review-index.html"
echo ""
echo "Contact sheets:"
echo "$OUT/_visual-audit/phase-1/hair-stylists"
